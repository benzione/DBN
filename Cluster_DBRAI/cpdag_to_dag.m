...
%
%  see Chikering (2002) : Learning equivalence classes of bayesian networks, JMLR2, pp475-479
%
...
for da=1:length(cpdag)
    ...
    while ~isempty(unprocessed)
      ...
      for i=1:length(unprocessed)
        nbr_parents(end+1)=length(parents(dag,unprocessed(i)));
      end
      [tmp, idx] = max(nbr_parents);
      node = unprocessed(idx);
      % Bugged line
      % dag(parents(dag,node),node)=0;
      % should be    
      dag(myintersect(parents(dag,node),unprocessed), node)=0;
